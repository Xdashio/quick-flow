...
export function getApiUrl() {
    const envUrl = import.meta.env?.VITE_API_URL || import.meta.env?.POS_API_URL;
    if (envUrl)
        return normalizeApiUrl(envUrl);
    return activeApiUrl || CLOUD_API_URL;
}
export async function getApiUrlCandidates() {
    const urls = [];
    ...
    urls.push(import.meta.env?.VITE_API_URL, import.meta.env?.POS_API_URL, CLOUD_API_URL, "http://localhost:3000/api");
    const seen = new Set();
    const ordered = [];
    if (activeApiUrl) {
...
export const posApi = {
    ...
    async completeCashSale(payload) {
        if (window.posApi?.completeCashSale)
            return window.posApi.completeCashSale(payload);
    ...
    async initiateMpesaStkSale(payload) {
    ...
    async completeMpesaTillSale(payload) {
    ...
    async completeMpesaStkSale(paymentId) {
    ...
    async openDrawer(args) {
        if (window.posApi?.openDrawer)
    ...
    async getPendingCount() {
        if (window.posApi?.getPendingCount)
            return window.posApi.getPendingCount();
        return 0;
    },
    async checkConnectivity() {
        if (window.posApi?.checkConnectivity)
            return window.posApi.checkConnectivity();
        try {
     ...
     */
    async getImageLocalPath(productId) {
